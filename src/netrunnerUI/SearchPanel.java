...
public class SearchPanel extends JPanel {
	...
	JPanel searchBar;
	JScrollPane scrolledResults;
	JPanel searchResults;
	ArrayList<BufferedImage> images;
	
	public SearchPanel(LayoutManager l) {
		super(l);
		searchBar = new JPanel();
		searchBar.setLayout(new GridBagLayout());
		
		searchResults = new JPanel();
		searchResults.add(new JLabel("Use the search bar to the left to look up cards"));
		scrolledResults = new JScrollPane(searchResults);
	...
	searchButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			remove(scrolledResults);
			searchResults.removeAll();
			searchResults.add(displayImageSearch(CardDatabase.getCardImages(search.getResults())));
			scrolledResults = new JScrollPane(searchResults);
			add(scrolledResults, BorderLayout.EAST);
			scrolledResults.setPreferredSize(new Dimension(getSearchResultWidth(), getHeight()));
	...
	this.add(scrolledResults, BorderLayout.EAST);
	}
	
	public JPanel displayImageSearch(ArrayList<BufferedImage> searchedImages) {
		if (searchedImages == null) {
			return new JPanel();
		}
		JPanel imagePanel = new JPanel();
		for (BufferedImage b : searchedImages) {
			imagePanel.add(new JLabel(new ImageIcon(b)));
		}
		return imagePanel;
	}
	
	public int getSearchResultWidth() {
		return this.getWidth() - searchBar.getWidth();
	}
}
